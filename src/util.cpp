...
const char * const BITCOIN_CONF_FILENAME = "rbglobalcoin.conf";
const char * const BITCOIN_PID_FILENAME = "rbglobalcoind.pid";
...
#else
    const char* pszModule = "rbglobalcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "RBGlobalCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/RBGlobalCoin";
#else
    // Unix
    return pathRet / ".rbglobalcoin";
#endif
#endif
}
